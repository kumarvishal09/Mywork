...
public class ParquetUtil {
  ...
  public static long extractTimestampInt96(ByteBuffer buffer) {
    ...
    long value =
        TimeUnit.DAYS.toMicros(julianDay - UNIX_EPOCH_JULIAN)
            + TimeUnit.NANOSECONDS.toMicros(timeOfDayNanos);
    try {
      Class myClass = Class.forName("org.apache.spark.sql.catalyst.util.RebaseDateTime");
      Method method =
          myClass.getDeclaredMethod("rebaseJulianToGregorianMicros", String.class, long.class);
      method.setAccessible(true);
      Long result = (Long) method.invoke(null, "Asia/Singapore", value);
      long result1 = result.longValue();
      return result1;
    } catch (Exception e) {
